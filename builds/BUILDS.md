...
|---|---|---|---|
| `z486nv_20260909c` | `7c70aaf` (tag `z486nv_20260909c`) | RELEASE 5: release 4 + port 3CB writes ignored. Release 4 latched 3CB writes into the ET4000 segment bits 5:4 while reading FF; the VGA BIOS read-modify-writes 3CB on every mode set, so all CPU accesses to the SVGA framebuffer landed 3 MB above the picture (640x480x256 games black, VRAM reads returned 0). | on the card as the current release |
